...
declare
  ...
  safe_action text := coalesce(nullif(trim(requested_action), ''), 'none');
begin
  ...
  set
    ...
    admin_notes = nullif(trim(coalesce(requested_admin_note, '')), ''),
    resolved_at = case when safe_status in ('resolved'::public.report_status, 'dismissed'::public.report_status) then now() else resolved_at end,
    updated_at = now()
  where id = target_report_id
  ...
  if safe_status in ('resolved'::public.report_status, 'dismissed'::public.report_status) then
    ...
    if report_row.reporter_id is not null then
      ...
      values (
        ...
        reporter_title,
        reporter_body,
        jsonb_build_object('reportId', target_report_id, 'reportStatus', safe_status, 'moderationAction', safe_action)
      );
    end if;
