-- Keep admin report actions, in-app admin messages, and archived moderation
-- history working together for the private beta admin panel.
...
begin
  if not exists (
    ...
    where conrelid = 'public.conversations'::regclass
      and conname = 'conversation_has_context'
  ) then
    alter table public.conversations
      add constraint conversation_has_context check (
        (
...
revoke all on function private.create_admin_report_message(uuid, uuid, uuid, text)
  from public, anon, authenticated;
