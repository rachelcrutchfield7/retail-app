...
| `1bacbcf6-bfb9-4a36-9e69-39e3b7e7b59b` | `99998e9d8b3e55c289ad96894d0ed7f00335fb74` | Corrected Android preview APK build finished successfully; physical-device smoke test pending. |

The first-device smoke test remains incomplete until corrected APK `1bacbcf6-bfb9-4a36-9e69-39e3b7e7b59b` opens successfully on Rachel's Android device without the Supabase startup warning.
...
Status: Fail on first build; second build opened but configuration warning appeared; corrected build retest pending.
Notes: First APK installed successfully, but launch failed immediately with the global error boundary. Second APK installed and opened, but showed "Startup needs attention" because the compiled Supabase public configuration was missing. Corrected APK `1bacbcf6-bfb9-4a36-9e69-39e3b7e7b59b` is available for retest.
Screenshot or recording:

### Launch the app

Status: Fail on first build; second build blocked at startup configuration warning; corrected build retest pending.
Notes: First APK displayed "Something went wrong" and "ReTail ran into a problem. Your account and listings are still safe." Second APK opened and showed "Startup needs attention" with missing Supabase configuration. Corrected APK `1bacbcf6-bfb9-4a36-9e69-39e3b7e7b59b` is available for retest.
Screenshot or recording:
