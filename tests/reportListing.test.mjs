...
test('admin report actions can resolve, dismiss, remove listings, remove messages, delete users, and notify both sides', () => {
  ...
  assert.match(sprint4, /\{canDeleteUser \? <Button title="Delete Account"/);
  assert.match(sprint4, /Report Queue/);
  assert.match(sprint4, /FilterChip label="Archived"/);
  assert.match(sprint4, /Move to Active/);
  assert.match(adminService, /AdminReportQueueMode = 'active' \| 'archived'/);
  assert.match(adminService, /mode === 'archived'/);
  assert.match(adminHook, /getListingReportQueue\(mode\)/);
