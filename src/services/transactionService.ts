...
function toTransaction(row: Row): Transaction {
  return {
    ...
    platform_fee_cents: typeof row.platform_fee_cents === 'number' ? row.platform_fee_cents : undefined,
    seller_amount_cents: typeof row.seller_amount_cents === 'number' ? row.seller_amount_cents : undefined,
    currency: optionalString(row.currency),
    stripe_payment_intent_id: optionalString(row.stripe_payment_intent_id),
    stripe_transfer_destination: optionalString(row.stripe_transfer_destination),
    stripe_latest_charge_id: optionalString(row.stripe_latest_charge_id),
    stripe_receipt_url: optionalString(row.stripe_receipt_url),
    paid_at: optionalString(row.paid_at),
    refunded_at: optionalString(row.refunded_at),
    payment_error: optionalString(row.payment_error),
