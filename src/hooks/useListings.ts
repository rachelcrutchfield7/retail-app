...
import { getListings, getRescueDonationListings } from '../services/listingService';
...
export function useRescueDonationListings(params: ListingQueryParams = {}) {
  const queryHash = useMemo(() => JSON.stringify(params), [params]);
  const query = useQuery<PaginatedListings, Error>({
    queryKey: [...queryKeys.listings, 'rescue-donations', queryHash],
    queryFn: () => getRescueDonationListings({ limit: cachePolicy.listings.pageSize, ...params }),
    staleTime: cachePolicy.listings.staleTime,
    gcTime: cachePolicy.listings.cacheTime,
  });
