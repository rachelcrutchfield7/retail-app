    ...
    if (activeTab === 'create') {
      return (
        <CreateListingScreen
          ...
          publishing={createListing.loading || updateListing.loading}
          onSignIn={() =>
            requestAuth({
              title: 'List pet supplies',
              body: 'Create an account to list, sell, or donate pet supplies locally.',
...
function updateListingInputFromForm(
  ...
  return {
    ...
    price: isDonation ? null : form.price.trim() || '$0',
    images: form.images,
    city: listing.city ?? listing.location.split(',')[0]?.trim() ?? profile?.city?.trim() ?? '',
    state: listing.state ?? listing.location.split(',')[1]?.trim() ?? profile?.state?.trim() ?? '',
    zip_code: listing.zipCode ?? profile?.zip_code?.trim() ?? '',
    pickup_available: form.pickup,
